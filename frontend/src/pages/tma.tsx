...
export default function TMAPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#030014] flex items-center justify-center">
        <div className="text-amber-400 animate-pulse">{t('loading')}</div>
      </div>
    );
  }

  const tabs: { id: TabId; label: string; icon: React.ReactNode }[] = [
    { id: 'overview', label: t('dashboard'), icon: <Activity className="w-4 h-4" /> },
    { id: 'worker', label: t('cta_worker'), icon: <Zap className="w-4 h-4" /> },
    { id: 'golden', label: t('gold_reserve_title'), icon: <Coins className="w-4 h-4" /> },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#030014] text-white pb-24">
      ...
      <div className="p-4">
        ...
        {tab === 'overview' && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-xl bg-white/5 border border-white/10 p-4">
                <div className="text-[10px] uppercase tracking-widest text-gray-500 font-bold mb-1">{t('stat_tasks')}</div>
            ...
            {!address && (
              <div className="rounded-xl bg-white/5 border border-white/10 p-4">
                <p className="text-center text-gray-500 text-xs mb-3">{t('connect_wallet')} → {t('gold_reserve_title')}</p>
                <WalletConnect />
              </div>
            )}
          </div>
        ...
        {tab === 'golden' && (
          <div className="space-y-4">
            {!address ? (
              <div className="rounded-xl bg-white/5 border border-white/10 p-4">
                <p className="text-center text-gray-500 text-xs mb-3">{t('connect_wallet')} → Golden Gateway</p>
                <WalletConnect />
              </div>
            ) : (
              <>
      ...
      {!isTelegramWebApp() && (
        <p className="text-center text-gray-500 text-xs py-4">Open in Telegram for full experience</p>
      )}
    </div>
  );
}
