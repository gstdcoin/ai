      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-lg font-bold text-white flex items-center gap-2 font-display">
            ...
            {t('pool_status', 'Pool Status') || 'GSTD/XAUt Pool Status'}
          </h3>
          <p className="text-xs text-gray-400 mt-1">
            {t('pool_backing', 'Token Backing Pool') || 'Token Backing Pool'}
          </p>
        </div>
        <button
      ...
      {loading && !poolStatus ? (
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between bg-black/20 px-3 py-2 rounded-lg">
            <span className="text-sm text-gray-400">{t('pool_health', 'Pool Health') || 'Pool Health'}:</span>
            <span className={`px-3 py-1 rounded-full text-xs font-semibold ${poolStatus.is_healthy
                ? 'bg-green-500/20 text-green-400 border border-green-500/30'
                : 'bg-red-500/20 text-red-400 border border-red-500/30'
              }`}>
              {poolStatus.is_healthy ? (t('healthy', 'Healthy') || '✓ Healthy') : (t('unhealthy', 'Unhealthy') || '✗ Unhealthy')}
            </span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-sm text-gray-400">{t('gstd_balance', 'GSTD Balance') || 'GSTD Balance'}:</span>
