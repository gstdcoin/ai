...
func (s *ResultService) SubmitResult(ctx context.Context, req SubmitResultRequest, validationService *ValidationService) error {
	...
	`, req.TaskID).Scan(
		&task.TaskID, &task.RequesterAddress, &task.Status, &task.AssignedDevice, &task.LaborCompensationTon,
	)
	if err != nil {
...
func (s *ResultService) ProcessPayment(ctx context.Context, taskID string) error {
	...
	err := s.db.QueryRowContext(ctx, `
		...
		FROM tasks WHERE task_id = $1
	`, taskID).Scan(
		&task.TaskID, &task.RequesterAddress, &task.AssignedDevice, &task.LaborCompensationTon, &task.Status,
	)
	if err != nil {
		return err
	}
