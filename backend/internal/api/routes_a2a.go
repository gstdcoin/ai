...
func agentsHandshake(deviceService *services.DeviceService, apiKeyService *services.APIKeyService, db *sql.DB, nodeService *services.NodeService) gin.HandlerFunc {
	return func(c *gin.Context) {
		handleAgentsHandshake(c, deviceService, apiKeyService, db, nodeService)
	}
}

//nolint:gocognit // NOSONAR
func handleAgentsHandshake(c *gin.Context, deviceService *services.DeviceService, apiKeyService *services.APIKeyService, db *sql.DB, nodeService *services.NodeService) {
	...
	wallet := resolveWalletAddress(c, req.WalletAddr, apiKeyService)
	...
	if nodeService != nil && wallet != "" {
		specs := map[string]interface{}{}
		if len(req.Capabilities) > 0 {
			specs["capabilities"] = strings.Join(req.Capabilities, ",")
		}
		_, _ = nodeService.RegisterNode(c.Request.Context(), wallet, "Agent-"+deviceID, specs, nil, nil, nil, false)
		_ = nodeService.UpdateHealthStats(c.Request.Context(), wallet, 100, 100, nil, nil)
	}
...
// resolveWalletAddress extracts wallet from request body, API keys, or headers
func resolveWalletAddress(c *gin.Context, reqWallet string, apiKeyService *services.APIKeyService) string {
	wallet := reqWallet
	if wallet == "" {
